...
mod arm_neon {
    ...
    pub unsafe fn gaussian_neon(
        ...
        let weights_h = precompute_gaussian_weights(src_w, dst_w, 1.0);
    ...
    unsafe fn process_pixel_neon_ps4(src: &[u8], wi: &WeightInfo, sums: &mut [f32; 4], max_w: i32) {
        ...
        for (k, &w) in wi.weights.iter().enumerate() {
            ...
            let v_f32 = unsafe {
                vcvtq_f32_u32(vmovl_u16(vget_low_u16(vmovl_u8(vreinterpret_u8_u32(v_u8)))))
            };
